...
class SharedCog(Cog, name="AAA3A_utils"):
    ...
    async def telemetrywithsentry(self, ctx: commands.Context, state: bool) -> None:
        ...
        self.display_sentry_manual_command = not self.sentry.sentry_enabled and (
    ...
    async def flags(self, ctx: commands.Context, *, content: str) -> None:
        ...
        async def _parse_arguments():
            ...
            result1 = await discord.ext.commands.converter.run_converters(
                context,
                converter,
                context.message.content[len(context.prefix) :],
                param,
            )
    ...
    async def getallfor(
        ...
        uptime = humanize_timedelta(timedelta=datetime.datetime.timezone.utcnow() - ctx.bot.uptime)
        ...
        context_table.add_row(
            "User permissions value (channel)",
            str(ctx.channel.permissions_for(ctx.author).value),
        )
            ...
            if page is not None:
                if count_page == 1:
                    to_html += message_html.replace(
                        ...
                        "{TIMESTAMP}",
                        str(ctx.message.created_at.strftime("%b %d, %Y %I:%M %p")),
                    )
                else:
                    to_html += (
